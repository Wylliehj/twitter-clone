...
def show_likes(user_id):
    messages = [message for message in g.user.likes]  
    likes = [like.message_id for like in Likes.query.filter_by(user_id=g.user.id)]

    return render_template('/users/likes.html', messages=messages, user=g.user, likes=likes)


def handle_like(msg_id):
    """Handles a user like/unlike, and interacts with the database accordingly"""
    user = g.user
    liked_messages = Likes.query.filter_by(user_id=user.id).all()

    for like in liked_messages:
        if like.message_id == msg_id:

            Likes.query.filter_by(id=like.id).delete()
            db.session.commit()  
            return redirect('/')

    liked = Likes(user_id=user.id, message_id=msg_id)
